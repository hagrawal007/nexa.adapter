...
namespace Nexa.Adapter.Services
{
    ...
    // <summary>
    /// Calls GET /customers/{customerId}/crm-insights
    /// </summary>
    public class CrmInsightsTool : ITool
    {
        private static readonly HttpClient _httpClient = new HttpClient
        {
            Timeout = TimeSpan.FromSeconds(30)
        ...
        public CrmInsightsTool(IConfiguration config)
        {
            // Configure this in appsettings.json, env var, etc.
            // Example: "ToolApi": { "BaseUrl": "https://localhost:5001" }
            _baseUrl = (config["BankDataApi:BaseUrl"] ?? "").TrimEnd('/');
        }

        public string Name => "CrmInsights";
        public string Description => "Fetch CRM insights for a customer by customerId.";
        ...
        public async Task<ToolResult> ExecuteAsync(ToolCall call)
        {
            try
            {
                if (string.IsNullOrWhiteSpace(_baseUrl))
                    return Error("BankDataApi:BaseUrl is not configured.");

                var rawCustomerId = call?.Args?.ContainsKey("customerId") == true
                    ? call.Args["customerId"]
                    : null;

                if (!int.TryParse(rawCustomerId, out var customerId))
                    return Error("Valid customerId (integer) is required.");

                var url = $"{_baseUrl}/customers/{customerId}/crm-insights";
    ...
    /// <summary>
    /// Calls GET /customers/{customerId}/behaviour
    /// </summary>
    public class CustomerBehaviourTool : ITool
    {
        private static readonly HttpClient _httpClient = new HttpClient
        {
            Timeout = TimeSpan.FromSeconds(30)
        ...
        public CustomerBehaviourTool(IConfiguration config)
        {
            _baseUrl = (config["BankDataApi:BaseUrl"] ?? "").TrimEnd('/');
        }

        public string Name => "CustomerBehaviour";
        public string Description => "Fetch behavioural data for a customer by customerId (risk/churn signals).";
        ...
        public async Task<ToolResult> ExecuteAsync(ToolCall call)
        {
            try
            {
                ...
                var url = $"{_baseUrl}/customers/{customerId}/behaviour";
